dashboard "tailpipe_plugin_pull_request_age_report" {

  title = "Tailpipe Plugin Pull Request Age Report"
  ...
  container {
    ...
    # Analysis
    card {
      sql   = query.tailpipe_aws_plugin_pull_request_external_count.sql
      width = 2
    }

    card {
      sql   = query.tailpipe_plugin_pull_request_total_days_count.sql
      width = 2
    }

    table {
      sql = query.tailpipe_plugin_pull_request_table.sql
...
query "tailpipe_aws_plugin_pull_request_external_count" {
  sql = <<-EOQ
    select
      'AWS Plugin' as label,
      case
    ...
    where
      ...
      and repository_full_name = 'turbot/tailpipe-plugin-aws'
...
query "tailpipe_plugin_pull_request_total_days_count" {
  sql = <<-EOQ
    select
      'Total' as label,
      case
    ...
    where
      query = '${local.dashboard_pull_request_search_query}'
      and repository_full_name like 'turbot/tailpipe-plugin-%'
...
query "tailpipe_plugin_pull_request_table" {
  sql = <<-EOQ
    select
