dashboard "steampipe_mod_pull_request_age_report" {

  title = "Steampipe Mod Pull Request Age Report"
  ...
  container {
    ...
    # Analysis
    card {
      sql   = query.steampipe_aws_compliance_mod_pull_request_external_count.sql
      width = 2
    }

    card {
      sql   = query.steampipe_mod_pull_request_total_days_count.sql
      width = 2
    }

    table {
      sql = query.steampipe_mod_pull_request_table.sql
...
query "steampipe_aws_compliance_mod_pull_request_external_count" {
  sql = <<-EOQ
    select
...
query "steampipe_mod_pull_request_total_days_count" {
  sql = <<-EOQ
    select
      'Total' as label,
      case
...
query "steampipe_mod_pull_request_table" {
  sql = <<-EOQ
    select
