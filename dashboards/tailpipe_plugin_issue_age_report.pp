dashboard "tailpipe_plugin_issue_age_report" {

  title = "Tailpipe Plugin Issue Age Report"
  ...
  container {
    ...
    # Analysis
    card {
      sql   = query.tailpipe_aws_plugin_issue_external_count.sql
      width = 2
    }

    card {
      sql   = query.tailpipe_plugin_issue_total_days_count.sql
      width = 2
    }

    table {
      sql = query.tailpipe_plugin_issue_table.sql
...
query "tailpipe_aws_plugin_issue_external_count" {
  sql = <<-EOQ
    select
      'AWS Plugin' as label,
      case
...
query "tailpipe_plugin_issue_total_days_count" {
  sql = <<-EOQ
    select
      'Total' as label,
      case
...
query "tailpipe_plugin_issue_table" {
  sql = <<-EOQ
    select
