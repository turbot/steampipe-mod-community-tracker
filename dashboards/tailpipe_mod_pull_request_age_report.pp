dashboard "tailpipe_mod_pull_request_age_report" {

  title = "Tailpipe Mod Pull Request Age Report"
  ...
  container {
    ...
    card {
      sql   = query.tailpipe_mod_pull_request_total_days_count.sql
      width = 2
    }

    table {
      sql = query.tailpipe_mod_pull_request_table.sql
...
query "tailpipe_mod_pull_request_total_days_count" {
  sql = <<-EOQ
    select
      'Total' as label,
      case
    ...
    where
      ...
      and repository_full_name like 'turbot/tailpipe-mod-%'
...
query "tailpipe_mod_pull_request_table" {
  sql = <<-EOQ
    select
