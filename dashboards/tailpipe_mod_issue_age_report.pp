dashboard "tailpipe_mod_issue_age_report" {

  title = "Tailpipe Mod Issue Age Report"
  ...
  container {
    ...
    # Analysis
    card {
      sql   = query.tailpipe_mod_issue_total_days_count.sql
      width = 2
    }

    table {
      sql = query.tailpipe_mod_issue_table.sql
...
query "tailpipe_mod_issue_total_days_count" {
  sql = <<-EOQ
    select
      'Total' as label,
      case
...
query "tailpipe_mod_issue_table" {
  sql = <<-EOQ
    select
