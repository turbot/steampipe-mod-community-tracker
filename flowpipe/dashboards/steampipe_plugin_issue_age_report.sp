dashboard "steampipe_plugin_issue_age_report" {

  title = "Steampipe Plugin Issue Age Report"
  ...
  container {
    ...
    # Analysis
    card {
      sql   = query.steampipe_aws_plugin_issue_external_count.sql
      width = 2
    }

    card {
      sql   = query.steampipe_plugin_issue_total_days_count.sql
      width = 2
    }

    table {
      sql = query.steampipe_plugin_issue_table.sql
...
query "steampipe_aws_plugin_issue_external_count" {
  sql = <<-EOQ
    select
      'AWS Plugin' as label,
      case
...
query "steampipe_plugin_issue_total_days_count" {
  sql = <<-EOQ
    select
      'Total' as label,
      case
...
query "steampipe_plugin_issue_table" {
  sql = <<-EOQ
    select
