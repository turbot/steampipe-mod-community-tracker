dashboard "flowpipe_core_pull_request_age_report" {
  ...
  container {
    ...
    card {
      sql   = query.flowpipe_sdk_external_pull_request_count.sql
      width = 2
    }

    card {
      sql   = query.flowpipe_fdw_external_pull_request_count.sql
      width = 2
    }
...
query "flowpipe_sdk_external_pull_request_count" {
  sql = <<-EOQ
    select
      'SDK' as label,
      case
