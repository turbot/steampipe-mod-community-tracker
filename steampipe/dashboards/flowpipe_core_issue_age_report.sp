dashboard "flowpipe_core_issue_age_report" {
  ...
  container {
    ...
    card {
      sql   = query.flowpipe_sdk_external_issue_count.sql
      width = 2
    }

    card {
      sql   = query.flowpipe_pipe_fittings_external_issue_count.sql
      width = 2
    }
...
query "flowpipe_core_issue_total_days_count" {
  sql = <<-EOQ
    ...
    from
      github_search_issue
    where
      query = '${local.dashboard_flowpipe_issue_search_query}'
...
query "flowpipe_sdk_external_issue_count" {
  sql = <<-EOQ
    select
      'SDK' as label,
      case
...
query "flowpipe_pipe_fittings_external_issue_count" {
  sql = <<-EOQ
    select
      'FDW' as label,
      case
